use crate::application::ports::{
    DirectoryRepository, EmbeddingRepository, PhotoEvaluationRepository, PhotoRepository,
    SettingsRepository, VectorRepository,
};
use crate::domain::models::{
    AiSettings, DimensionScore, Directory, Photo, PhotoEvaluation, PhotoSortOrder,
};
...
async fn attach_latest_evaluations(db: &Pool<Sqlite>, photos: &mut [Photo]) -> Result<()> {
    if photos.is_empty() {
        return Ok(());
    }
